...
@media(max-width:980px){#lf-panel{width:calc(100vw-20px);transform:none;top:20px;left:10px;right:auto}.lf-panel-body{min-height:260px}.lf-panel-text{min-height:200px}}`;
  ...
  let lastMX=0,lastMY=0,ctrlPressTimer=null,ctrlDown=false; document.addEventListener('mousemove',e=>{ lastMX=e.clientX;lastMY=e.clientY; },{ passive:true }); document.addEventListener('keydown',e=>{ if(e.key==='Control'||e.key==='Meta'){ ctrlDown=true; clearTimeout(ctrlPressTimer); ctrlPressTimer=setTimeout(()=>{ if(!ctrlDown) return; const el=document.elementFromPoint(lastMX,lastMY); if(!el||el.closest('[id^="lf-"]')) return; const sel=window.getSelection(); const selText=(sel&&!sel.isCollapsed)?sel.toString().trim():''; if(selText){ handleExplainPoint({ clientX:lastMX||window.innerWidth/2,clientY:lastMY||window.innerHeight/2,ctrlKey:true,metaKey:e.key==='Meta' }); } else if(!el.closest('input,textarea,[contenteditable]')){ handleExplainPoint({ clientX:lastMX,clientY:lastMY,ctrlKey:e.key==='Control',metaKey:e.key==='Meta' }); } },250); } if(e.key!=='Control'&&e.key!=='Meta'){ clearTimeout(ctrlPressTimer); ctrlDown=false; } }); document.addEventListener('keyup',e=>{ if(e.key==='Control'||e.key==='Meta') ctrlDown=false; });
