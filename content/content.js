...
(function () {
  ...
  const CJK_TARGETS = ['zh-CN','zh-TW'];
  const JA_CHARS = /[぀-ゟ゠-ヿ]/; // 日文假名
...
.lf-collapse-btn{display:inline-flex!important;align-items:center!important;justify-content:center!important;width:28px!important;height:28px!important;min-width:0!important;min-height:0!important;background:linear-gradient(135deg,rgba(35,30,55,0.94),rgba(18,16,36,0.96));backdrop-filter:blur(10px) saturate(1.4);-webkit-backdrop-filter:blur(10px) saturate(1.4);border:1px solid rgba(124,92,252,0.35);box-shadow:0 2px 14px rgba(0,0,0,0.45),0 0 0 1px rgba(0,0,0,0.3),0 0 20px rgba(124,92,252,0.12),inset 0 1px 0 rgba(255,255,255,0.06);border-radius:50%!important;color:rgba(255,255,255,0.6);cursor:pointer;padding:0!important;transition:color 0.2s,background 0.2s,border-color 0.2s;animation:lf-bob-x 2.4s ease-in-out infinite}
...
@media(max-width:980px){#lf-panel{width:calc(100vw-20px);transform:none;top:20px;left:10px;right:auto}.lf-panel-body{min-height:260px}.lf-panel-text{min-height:200px}}`;
  ...
  function isTranslatable(text) { const t = text.trim(); const targetLang = settings.targetLang||'zh-CN'; const minLen=CJK_TARGETS.includes(targetLang)?3:1; if (!t||t.length<minLen) return false; if (RE_SYMBOLS.test(t)) return false; if (RE_URL.test(t)) return false; if (/^Test[: ]/i.test(t)) return false; if (/^[a-z]+-[a-z]+-[a-z]+$/.test(t)&&t.length>15) return false; if (CJK_TARGETS.includes(targetLang)&&/[一-鿿]/.test(t)&&!JA_CHARS.test(t)) { let cjk=0; for (const ch of t) { if (ch>='一'&&ch<='鿿') cjk++; } if (cjk/t.length>0.3) return false; if (cjk>0&&!/[a-zA-Z]{3,}/.test(t)) return false; } return RE_LETTER.test(t); }
  ...
  function handleExplainPoint(e) {
    ...
    if (explainCache.has(phrase)){ showBubble(e.clientX,e.clientY,cursorWord||phrase); updateBubble(explainCache.get(phrase)); return; }
  ...
  setInterval(restoreLangOptions,10000);
  ...
  function createPanel(){
    ...
    let panelTimer=null;
    function splitText(txt,limit=300){
      if(txt.length<=limit) return [txt];
      const chunks=[];
      // 优先按段落分
      const paras=txt.split(/\n\s*\n/);
      for(const p of paras){
        if(p.length<=limit){ chunks.push(p); continue; }
        // 按句号分
        const sents=p.split(/(?<=[。！？.!?])\s*/);
        let cur='';
        for(const s of sents){
          if((cur+s).length>limit&&cur){ chunks.push(cur.trim()); cur=s; }
          else cur+=s;
        }
    ...
    const doPanelTranslate=async()=>{
      ...
      try{
        ...
        let aborted=false; panelAbort=()=>{ aborted=true; };
        const results=await Promise.all(chunks.map((chunk,i)=>{
          if(aborted) return '';
          if(translationCache.has(chunk)) return translationCache.get(chunk);
          const tokLimit=Math.min(chunk.length*3+100,800);
          return new Promise((resolve)=>{ chrome.runtime.sendMessage({ type:'PANEL_TRANSLATE', text:chunk, maxTokens:tokLimit, sourceLang:srcSel.value, targetLang:tgtSel.value, settings:{ apiKey:cfg.apiKey, apiUrl:cfg.apiUrl, model:cfg.model } }, resp=>{
            if(aborted){ resolve(chunk); return; }
            if(resp?.success){ translationCache.set(chunk,resp.translation); markCacheDirty(); if(resp.usage) addPageTokens(resp.usage); resolve(resp.translation); }
            else resolve('⚠️ '+(resp?.error||'翻译失败'));
          });});
        }));
        if(!aborted){ out.value=results.join('\n\n'); translationCache.set(txt,out.value); markCacheDirty(); }
      }catch(e){ out.value='⚠️ 翻译出错'; }
      btn.textContent='🔄 翻译'; btn.disabled=false; panelAbort=null;
    };
    let panelAbort=null;
